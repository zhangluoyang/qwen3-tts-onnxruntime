...
from src.custom_model import CustomQwen3TTSOnnxModel


MODEL_DIR = "/home/zhang/.cache/modelscope/hub/models/Qwen/Qwen3-TTS-12Hz-1.7B-CustomVoice"
ONNX_DIR = "onnx_custom_fp16"
OUTPUT_DIR = Path("outputs")

SPEAKER = "Vivian"
LANGUAGE = "chinese"
INSTRUCT = "语气温柔自然，情绪平稳，像是在安静地讲故事。"
KV_WINDOW_FRAMES = 128
KV_WINDOW_MAX_FRAMES = 256
...
def main() -> None:
    ...
    print(f"supported_speakers={model.supported_speakers()}")

    # nonstream = model.generate_custom_voice(
        # text=NONSTREAM_TEXT,
        # speaker=SPEAKER,
        # language=LANGUAGE,
        # instruct=INSTRUCT)
    # save_audio(OUTPUT_DIR / "custom_voice_nonstream.wav", nonstream.audio, nonstream.sample_rate)

    stream_chunks = model.stream_custom_voice(
        STREAM_TEXT_DELTAS,
        speaker=SPEAKER,
        language=LANGUAGE,
        instruct=INSTRUCT,
    ...
    save_audio(OUTPUT_DIR / "custom_voice_stream.wav", stream_audio, stream_sr)
